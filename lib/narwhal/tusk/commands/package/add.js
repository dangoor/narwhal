...
var action = exports.action = function (options) {
    ...
    // if we are adding a package to a sea based on a tusk:// URL we
    // inherit the install method
    if(pkg.uri.protocol=="tusk" && catalog.getType()=="sea") {
        addOptions.installMethod = "inherit";
    }
    

    catalog.addPackage(pkg,addOptions);
