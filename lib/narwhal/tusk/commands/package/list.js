...
parser.action(function (options) {
    ...
    Object.keys(packages.catalog).forEach(function (name) {
        ...
        line = "  \0green(" + name + "\0) " + ((sea.isPackageInstalled(name))?"\0cyan("+directory+"\0)":directory);
    ...
    print("Missing dependent (based on package.json) packages:");
    ...
    dependentPackageNames.forEach(function(name) {
        ...
        if(seaCatalog.hasPackage(name)) {
            ...
            line += ((sea.isPackageInstalled(name))?"\0cyan("+pkg.getPath()+"\0)":pkg.getPath());
        } else {
            line += "?";
        }
