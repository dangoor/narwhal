...
Package.prototype.getModulePath = function(selector) {

    // TODO: Maybe we should be using require() for this?
    //       Should be no problem since tusk runs in the context
    //       of the SEA which exposes all installed packages/modules.

    var parts = selector.split(":");
    if(parts.length==2) {
        // we have a <package>:<module> selector
        
        var pkg = SEA.getActive().getPackage(parts[0]);
        if(!pkg || !pkg.exists()) {
            throw "could not find package in sea: " + parts[0];
        }
        return pkg.getModulePath(parts[1]);
        
    } else {
        // we have a simple <module> selector
        return this.getLibPath().join(selector + ".js");
    }
}
...
Package.prototype.install = function(sea, options) {
    ...
    if(options.name) {
        // the package will be installed using the provided name
        path = sea.getPackagesPath().join(this.getName());
    } else {
        // the package will be installed using the fully qualified <catalog>/<package>/<revision> URI
        path = sea.getPackagePath(this);
    }
    ...
    if(!this.catalogOrigin || !util.has(this.catalogOrigin, "installMethod")) {
        installMethod = "copy";
    } else {
        installMethod = this.catalogOrigin.installMethod;
    }

    switch(installMethod) {
        case "link":
            print("Linked package at '"+this.getPath()+"' to: " + path);
    ...
    dependencies.forEach(function(dependency) {
        ...
        var dependencyOptions = util.copy(options);
        delete dependencyOptions.name;
        
        dependency.install(sea,dependencyOptions);
    });
