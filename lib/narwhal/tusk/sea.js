...
Sea.prototype.forEachPackage = function(callback) {
    // Iterate all packages installed in the sea
    util.keys(PACKAGES.catalog).forEach(function(name) {
        callback(name, PACKAGES.catalog[name]);
    });        
}


Sea.prototype.getPackage = function(pkg) {
    var path = this.getPackagePath(pkg);
    if(!path.exists()) {
        return null;
    }
...
Sea.prototype.getPackagePath = function(pkg) {
    ...
    if(parts.length==3) {
        ...
        return this.path.join("packages", pkg);
    }
}


Sea.prototype.getBuildPath = function() {
    return this.getPackageManifest().getBuildPath();
}
