...
exports.load = function (prefixes) {
    ...
    var catalog = {};
    var pkgCatalog = [];
    var root = exports.read(prefixes, catalog, pkgCatalog);
    ...
    pkgCatalog.sort(function(a, b) {
        if(a[0]==b[0]) return 0;
        if(a[0].length>b[0].length) return -1;
        return 1;
    });
    // add package catalog info to the loader
...
*/
exports.read = function read(prefixes, catalog, pkgCatalog) {
    ...
    while (prefixes.length) {
        ...
        while (queue.length) {
            ...
            // packages/dependencies/<catalog>/<package>/<revision>
            
            if(name=="dependencies") {

                packageDirectory.listPaths().forEach(function(catalogDirectory) {
                    if(catalogDirectory.isDirectory()) {
                        catalogDirectory.listPaths().forEach(function(packageDirectory) {
                            if(packageDirectory.isDirectory()) {
                                packageDirectory.listPaths().forEach(function(revisionDirectory) {
                                    
                                    // TODO: select package revision based on package.json
        
                                    queue.push([revisionDirectory,{
                                        name: packageDirectory.basename(),
                                        catalog: catalogDirectory.basename(),
                                        revision: revisionDirectory.basename()
                                    }]);
                ...
                try {
                    ...
                    if(dependencyInfo) {
                        ...
                        packageDatum.name = name = dependencyInfo.catalog + "/" + dependencyInfo.name + "/" + dependencyInfo.revision;
                        
                        // TODO: name package by alias if defined in package.json
                    }
                    ...
                    pkgCatalog.push([packageDatum.directory, pkgCatalogDatum]);
                    ...
                    if(util.has(packageDatum, "dependencies")) {
                        util.forEach(packageDatum.dependencies, function(dependency) {
                            ...
                            if(util.isArrayLike(dependency)) {
                                pkgCatalogDatum["packages"][dependency[0]] = exports.normalizePkgURL(dependency[1]);
                            }
                        });
                    }
    ...
    try {
        if (packageDatum.dependencies) {
            packageDatum.dependencies.forEach(function (dependency) {
                if(util.isArrayLike(dependency)) {
                    scan(catalog, exports.normalizePkgURL(dependency[1]));
                } else {
...
var _topo = function _topo(graph, name, visited) {
    ...
    for (var i = 0; i < dependencies.length; i++) {
        ...
        if(util.isArrayLike(dependency)) {
            dependency = exports.normalizePkgURL(dependency[1]);
        }
...
exports.normalizePkgURL = function(url) {
    if(!url) {
        return url;
    }
    var parts = [],
    ...
    if(url.protocol!="tusk") {
        throw "only tusk URL's are supported to locate dependencies at this time";
    }
    if(url.domain) {
        parts.push(url.domain);
    }
    parts.push(url.path.substr(1));
