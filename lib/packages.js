                    ...
                    if(util.has(packageDatum, "dependencies")) {
                        util.forEach(packageDatum.dependencies, function(dependency) {
                            ...
                            if(util.isArrayLike(dependency)) {
                                ...
                                // keep track of sea dependencies so we can map when resolving dependency locators
                                if(isSeaPackage) {
                                    seaDependencyMap[pkgCatalogDatum["packages"][dependency[0]]] = dependency[0];
                                }
                            }
                        });
                    }
        ...
        while (stack.length) {
            ...
            if (Object.prototype.hasOwnProperty.call(arrived, name)) {
                ...
                for (var i = 0; i < length; i++) {
                    ...
                    if(util.isArrayLike(dependency)) {
                        dependency = exports.normalizePkgLocator(dependency[1]);
                    }
...
exports.normalizePkgLocator = function(locator) {
    if(!locator) {
        return locator;
    }
    var uri;
