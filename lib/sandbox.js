...
exports.MultiLoader = function (options) {
    ...
    self.resolvePkg = function(id, baseId, pkg, basePkg) {
        return exports.resolvePkg(self, id, baseId, pkg, basePkg);
    };
...
exports.resolvePkg = function(loader, id, baseId, pkg, basePkg) {
    ...
    if(loader["pkgCatalog"]) {
        ...
        // based on pkgId determine owning package
        // this compares the pkgId (absolute path) to the package paths
        // if pkgId is not an absolute path it already is a fully-qualified package ID for the sea
        if(system.fs.Path(callingPkgId).exists()) {
            found = false;
            loader.pkgCatalog.forEach(function(pkgInfo) {
                if(found) {
                    return;
                }
